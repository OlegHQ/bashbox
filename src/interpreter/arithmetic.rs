    ...
    match expr {
        ...
        bast::ArithmeticExpr::UnaryAssignment(op, target) => {
            ...
            let (return_value, store_value) = match op {
                bast::UnaryAssignmentOperator::PrefixIncrement => (current + 1, current + 1),
                bast::UnaryAssignmentOperator::PrefixDecrement => (current - 1, current - 1),
                bast::UnaryAssignmentOperator::PostfixIncrement => (current, current + 1),
                bast::UnaryAssignmentOperator::PostfixDecrement => (current, current - 1),
            };
...
fn resolve_target(
    ...
    match target {
        bast::ArithmeticTarget::Variable(name) => resolve_variable(ctx, name, &mut HashSet::new()),
...
fn parse_value_as_arith(ctx: &mut InterpreterContext, value: &str) -> Result<i64, ArithmeticError> {
    ...
    let parsed = brush_parser::arithmetic::parse(value).map_err(|e| {
        ArithmeticError::new(
            format!("syntax error in expression (error token is \"{}\")", value),
            String::new(),
            format!("bash: {}\n", e),
            false,
        )
...
mod tests {
    ...
    fn test_apply_binary_op() {
        assert_eq!(
            apply_binary_op(5, 3, &bast::BinaryOperator::Add).unwrap(),
            8
        );
        assert_eq!(
            apply_binary_op(5, 3, &bast::BinaryOperator::Subtract).unwrap(),
            2
        );
        assert_eq!(
            apply_binary_op(5, 3, &bast::BinaryOperator::Multiply).unwrap(),
            15
        );
        assert_eq!(
            apply_binary_op(6, 3, &bast::BinaryOperator::Divide).unwrap(),
            2
        );
        assert_eq!(
            apply_binary_op(6, 3, &bast::BinaryOperator::Modulo).unwrap(),
            0
        );
        assert_eq!(
            apply_binary_op(5, 3, &bast::BinaryOperator::LessThan).unwrap(),
            0
        );
        assert_eq!(
            apply_binary_op(3, 5, &bast::BinaryOperator::LessThan).unwrap(),
            1
        );
        assert_eq!(
            apply_binary_op(5, 5, &bast::BinaryOperator::Equals).unwrap(),
            1
        );
    }
    ...
    fn test_evaluate_simple_expressions() {
        ...
        with_ctx!(&mut state, |ctx| {
            ...
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "1+2", false, None).unwrap(),
                3
            );
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "10-3", false, None).unwrap(),
                7
            );
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "2*3", false, None).unwrap(),
                6
            );
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "10/3", false, None).unwrap(),
    ...
    fn test_evaluate_variable() {
        ...
        with_ctx!(&mut state, |ctx| {
            ...
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "x+1", false, None).unwrap(),
                43
            );
        });
    }
    ...
    fn test_evaluate_assignment() {
        ...
        with_ctx!(&mut state, |ctx| {
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "x=5", false, None).unwrap(),
                5
            );
    ...
    fn test_evaluate_ternary() {
        ...
        with_ctx!(&mut state, |ctx| {
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "1 ? 10 : 20", false, None).unwrap(),
                10
            );
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "0 ? 10 : 20", false, None).unwrap(),
                20
            );
        });
    }
    ...
    fn test_evaluate_increment_decrement() {
        ...
        with_ctx!(&mut state, |ctx| {
            ...
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "++x", false, None).unwrap(),
                6
            );
    ...
    fn test_logical_short_circuit() {
        ...
        with_ctx!(&mut state, |ctx| {
            ...
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "1 || 0", false, None).unwrap(),
                1
            );
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "0 || 0", false, None).unwrap(),
                0
            );

            // LogicalAnd short-circuits when left is false
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "0 && 1", false, None).unwrap(),
                0
            );
            assert_eq!(
                evaluate_arithmetic(&mut ctx, "1 && 1", false, None).unwrap(),
                1
            );
        });
    }
