...
use crate::commands::registry::dispatch_external_command;
use crate::commands::types::{CommandContext, FetchFn};
use crate::interpreter::builtin_dispatch::{
    dispatch_builtin, BuildExportedEnvFn, BuiltinDispatchContext, ExecuteUserScriptFn, RunCommandFn,
};
use crate::interpreter::control_flow::{
    execute_for, execute_if, execute_until, execute_while, CaseTerminator, ForResult,
};
use crate::interpreter::errors::{ControlFlowError, ErrexitError, InterpreterError};
use crate::interpreter::functions::execute_function_def;
use crate::interpreter::helpers::condition::ConditionResult;
use crate::interpreter::interpreter::{
    check_command_limit, should_trigger_errexit, update_exit_code, FileSystem as SyncFileSystem,
};
...
pub struct ExecutionEngine<'a> {
    ...
    async_fs: Arc<dyn crate::fs::FileSystem>,
...
impl<'a> ExecutionEngine<'a> {
    ...
    pub fn new(
        ...
        sync_fs: &'a dyn SyncFileSystem,
        async_fs: Arc<dyn crate::fs::FileSystem>,
        runtime: tokio::runtime::Handle,
    ) -> Self {
        let fetch_fn = create_secure_fetch_fn(NetworkConfig::default(), ureq_fetch_fn());
        Self {
            limits,
            fs: sync_fs,
            async_fs,
    ...
    fn run_external_command(
        ...
        let env = _state.env.clone();
        let ctx = CommandContext {
            args: args.to_vec(),
            stdin: stdin.to_string(),
            cwd: _state.cwd.clone(),
            env,
            fs: self.async_fs.clone(),
            exec_fn: None,
            fetch_fn: Some(self.fetch_fn.clone()),
        };
        let out = tokio::task::block_in_place(|| {
            self.runtime
                .block_on(dispatch_external_command(cmd_name, ctx))
        });
        Ok(ExecResult::new(out.stdout, out.stderr, out.exit_code))
    }

    fn run_command_invocation(
        &self,
        ...
        let run_command: RunCommandFn =
            &|st, n, a, q, in_, sk, p, fd| self.run_command_invocation(st, n, a, q, in_, sk, p, fd);
        let build_exported_env: BuildExportedEnvFn = &|| std::collections::HashMap::new();
        let execute_user_script: ExecuteUserScriptFn =
            &|_, _, _| ExecResult::new(String::new(), String::new(), 0);
        ...
        match dispatch_builtin(
            ...
            Some(r) => Ok(r),
            None => self.run_external_command(ctx.state, cmd_name, args, stdin),
        }
    }
    ...
    pub fn execute_compound_command(
        ...
        match compound {
                ...
                if let Some(ref elses) = if_cmd.elses {
                    for else_clause in elses {
                        if let Some(ref cond) = else_clause.condition {
                            // elif clause
                            let cond_items: Vec<&bast::CompoundListItem> = cond.0.iter().collect();
                            ...
                            else_body = Some(else_clause.body.0.iter().collect());
                        }
                    }
                }
            ...
            bast::CompoundCommand::CaseClause(case_cmd) => {
                ...
                let case_items: Vec<
                    crate::interpreter::control_flow::CaseItem<bast::Word, bast::CompoundListItem>,
                > = case_cmd
                    .cases
                    .iter()
                    .map(|ci| {
            ...
            bast::CompoundCommand::Subshell(subshell_cmd) => {
                let body_items: Vec<&bast::CompoundListItem> = subshell_cmd.list.0.iter().collect();
                execute_subshell(state, &body_items, Some(stdin), |state, item| {
                    self.execute_compound_list_item(state, item)
                })
            }

            bast::CompoundCommand::BraceGroup(group_cmd) => {
                let body_items: Vec<&bast::CompoundListItem> = group_cmd.list.0.iter().collect();
                execute_group(state, &body_items, Some(stdin), |state, item| {
                    self.execute_compound_list_item(state, item)
                })
            }
