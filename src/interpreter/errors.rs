...
impl ArithmeticError {
    pub fn new(message: String, stdout: String, stderr: String, fatal: bool) -> Self {
        Self {
            ...
            stdout,
            stderr: default_stderr(stderr, &message),
            fatal,
        }
    }
...
impl_error!(ErrexitError, Errexit, |e: &ErrexitError| format!(
    "errexit: command exited with status {}",
    e.exit_code
));
impl_error!(NounsetError, Nounset, |e: &NounsetError| format!(
    "{}: unbound variable",
    e.var_name
));
impl_error!(ExitError, Exit, |_: &ExitError| "exit");
impl_error!(ArithmeticError, Arithmetic, |e: &ArithmeticError| e
    .message
    .clone());
impl_error!(
    BadSubstitutionError,
    BadSubstitution,
    |e: &BadSubstitutionError| e.message.clone()
);
impl_error!(GlobError, Glob, |e: &GlobError| format!(
    "no match: {}",
    e.pattern
));
impl_error!(
    BraceExpansionError,
    BraceExpansion,
    |e: &BraceExpansionError| e.message.clone()
);
impl_error!(
    ExecutionLimitError,
    ExecutionLimit,
    |e: &ExecutionLimitError| e.message.clone()
);
impl_error!(SubshellExitError, SubshellExit, |_: &SubshellExitError| {
    "subshell exit"
});
impl_error!(PosixFatalError, PosixFatal, |_: &PosixFatalError| {
    "posix fatal error"
});
