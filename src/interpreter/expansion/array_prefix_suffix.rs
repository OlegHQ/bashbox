...
pub fn apply_prefix_suffix_to_array(
    ...
    ArrayPrefixSuffixResult::new(format_element_list(
        values, is_star, prefix, suffix, &state.env,
    ))
}
...
pub fn apply_pattern_removal_with_prefix_suffix(
    ...
    let processed: Vec<String> = values
        ...
        .collect();
    ArrayPrefixSuffixResult::new(format_element_list(
        processed, is_star, prefix, suffix, &state.env,
    ))
}
...
pub fn handle_array_default_value(
    ...
    if !should_use_alternate {
        if !elements.is_empty() {
            ...
            return Some(ArrayPrefixSuffixResult::new(format_element_list(
                values, is_star, "", "", &state.env,
            )));
        }
...
pub fn check_array_assign_default(
    ...
    let values: Vec<String> = elements.into_iter().map(|(_, v)| v).collect();
    let result =
        ArrayPrefixSuffixResult::new(format_element_list(values, is_star, "", "", &state.env));
