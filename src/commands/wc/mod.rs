...
mod tests {
    ...
    async fn test_wc_chars_only() {
        let ctx =
            make_ctx_with_files(vec!["-c", "/test.txt"], vec![("/test.txt", "hello\n")]).await;
        let cmd = WcCommand;
        let result = cmd.execute(ctx).await;
    ...
    async fn test_wc_chars_m_flag() {
        let ctx =
            make_ctx_with_files(vec!["-m", "/test.txt"], vec![("/test.txt", "hello\n")]).await;
        let cmd = WcCommand;
        let result = cmd.execute(ctx).await;
        assert!(result.stdout.trim().ends_with("6 /test.txt"));
    }
    ...
    async fn test_wc_long_lines() {
        let ctx =
            make_ctx_with_files(vec!["--lines", "/test.txt"], vec![("/test.txt", "a\nb\n")]).await;
        let cmd = WcCommand;
        let result = cmd.execute(ctx).await;
    ...
    async fn test_wc_long_bytes() {
        let ctx =
            make_ctx_with_files(vec!["--bytes", "/test.txt"], vec![("/test.txt", "hi\n")]).await;
        let cmd = WcCommand;
        let result = cmd.execute(ctx).await;
