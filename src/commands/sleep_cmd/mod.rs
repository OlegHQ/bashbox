...
mod tests {
    ...
    #[test]
    fn test_parse_short() {
        assert_eq!(parse_duration("0.01"), Some(10.0));
    }
    #[test]
    ...
    async fn test_missing() {
        let r = SleepCommand.execute(make_ctx(vec![])).await;
        assert_eq!(r.exit_code, 1);
        assert!(r.stderr.contains("missing operand"));
    }
    #[tokio::test(flavor = "multi_thread")]
    async fn test_invalid() {
        let r = SleepCommand.execute(make_ctx(vec!["abc"])).await;
        assert_eq!(r.exit_code, 1);
        assert!(r.stderr.contains("invalid time interval"));
    }
    #[tokio::test(flavor = "multi_thread")]
    async fn test_bad_suffix() {
        let r = SleepCommand.execute(make_ctx(vec!["1x"])).await;
        assert_eq!(r.exit_code, 1);
    }
    #[tokio::test(flavor = "multi_thread")]
    async fn test_help() {
        let r = SleepCommand.execute(make_ctx(vec!["--help"])).await;
        assert!(r.stdout.contains("sleep"));
        assert!(r.stdout.contains("delay"));
    }
    #[tokio::test(flavor = "multi_thread")]
    async fn test_short_sleep() {
        let r = SleepCommand.execute(make_ctx(vec!["0.001"])).await;
