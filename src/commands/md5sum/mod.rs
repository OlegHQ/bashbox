...
use crate::commands::arg_helpers::{files_or_stdin, invalid_option, wants_help};
    ...
    for arg in args {
        match arg.as_str() {
            ...
            a if a.starts_with('-') && a != "-" => {
                return CommandResult::with_exit_code("".into(), invalid_option(name, &a[1..2]), 1);
            }
            _ => files.push(arg.clone()),
        }
    }
    let files = files_or_stdin(files);
    ...
    if check {
        ...
        for file in &files {
            let content = if file == "-" {
                ...
                let path = resolve_under_cwd(&ctx.cwd, file);
                ctx.fs.read_file(&path).await.ok()
            };
            let content = match content {
                Some(c) => c,
                None => {
                    return CommandResult::with_exit_code("".into(), no_such_file(name, file), 1)
                }
            };
            ...
            for line in content.lines() {
                ...
                let file_data = if target_file == "-" {
                    ...
                    let path = resolve_under_cwd(&ctx.cwd, target_file);
