...
impl Command for ChmodCommand {
    ...
    async fn execute(&self, ctx: CommandContext) -> CommandResult {
            ...
            match args[idx].as_str() {
                ...
                _ => {
                    return CommandResult::with_exit_code(
                        "".into(),
                        invalid_option("chmod", &args[idx][1..]),
                        1,
                    );
                }
            }
        ...
        let numeric_mode = if is_numeric {
            Some(u32::from_str_radix(mode_arg, 8).unwrap_or(0))
        } else {
            None
        };
        if !is_numeric {
        ...
        for file in files {
            ...
            let mv = if is_numeric {
                numeric_mode.unwrap_or(0)
            } else {
...
async fn chmod_rec(
    ...
    let entries = match read_dir_sorted(ctx.fs.as_ref(), dir).await {
        Ok(e) => e,
        Err(_) => return,
    };
    for entry in &entries {
        let fp = child_path(dir, entry);
        let mv = if is_numeric {
            nm.unwrap_or(0)
        } else {
            ctx.fs
                .stat(&fp)
                .await
                .map(|s| parse_mode(sm, s.mode).unwrap_or(0o644))
                .unwrap_or(0o644)
        };
        let _ = ctx.fs.chmod(&fp, mv).await;
        if v {
            out.push_str(&format!("mode of '{}' changed to {:04o}\n", fp, mv));
        }
        if let Ok(st) = ctx.fs.stat(&fp).await {
            if st.is_directory {
                Box::pin(chmod_rec(ctx, &fp, is_numeric, nm, sm, v, out)).await;
            }
        }
    }
}
...
fn parse_mode(mode_str: &str, current_mode: u32) -> Result<u32, String> {
    ...
    for part in mode_str.split(',') {
        ...
        let caps = re
            .captures(part)
            .ok_or_else(|| format!("Invalid mode: {}", mode_str))?;
        ...
        match op {
            ...
            "=" => {
                if perms.contains('s') {
                    if who.contains('u') {
                        mode = (mode & !0o4000) | (sb & 0o4000);
                    }
                    if who.contains('g') {
                        mode = (mode & !0o2000) | (sb & 0o2000);
                    }
                }
                if perms.contains('t') {
                    mode = (mode & !0o1000) | (sb & 0o1000);
                }
            }
            _ => {}
        }
...
mod tests {
    ...
    async fn test_chmod_octal() {
        let c = make_ctx(vec!["755", "/t.txt"], vec![("/t.txt", "h")]).await;
        assert_eq!(ChmodCommand.execute(c).await.exit_code, 0);
    }
    #[tokio::test(flavor = "multi_thread")]
    async fn test_chmod_missing() {
        let c = make_ctx(vec![], vec![]).await;
        let r = ChmodCommand.execute(c).await;
        assert_eq!(r.exit_code, 1);
        assert!(r.stderr.contains("missing"));
    }
    #[tokio::test(flavor = "multi_thread")]
    async fn test_chmod_nofile() {
        let c = make_ctx(vec!["755", "/x"], vec![]).await;
        let r = ChmodCommand.execute(c).await;
        assert_eq!(r.exit_code, 1);
        assert!(r.stderr.contains("No such file"));
    }
    #[tokio::test(flavor = "multi_thread")]
    async fn test_chmod_invalid() {
        let c = make_ctx(vec!["xyz", "/t.txt"], vec![("/t.txt", "h")]).await;
        let r = ChmodCommand.execute(c).await;
        assert_eq!(r.exit_code, 1);
        assert!(r.stderr.contains("invalid mode"));
    }
    #[tokio::test(flavor = "multi_thread")]
    async fn test_chmod_help() {
        let c = make_ctx(vec!["--help"], vec![]).await;
        let r = ChmodCommand.execute(c).await;
        assert!(r.stdout.contains("chmod"));
    ...
    fn test_parse_symbolic() {
        ...
        assert_eq!(parse_mode("a+x", 0o644).unwrap(), 0o755);
        assert_eq!(parse_mode("g-w", 0o664).unwrap(), 0o644);
        assert_eq!(parse_mode("u=rwx", 0o644).unwrap(), 0o744);
    }
}
