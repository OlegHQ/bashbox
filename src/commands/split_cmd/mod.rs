    ...
    async fn execute(&self, ctx: CommandContext) -> CommandResult {
        ...
        while i < args.len() {
                ...
                && arg[2..].chars().all(|c| c.is_ascii_digit())
            {
                ...
                match args[i + 1].parse::<usize>() {
                    ...
                    _ => {
                        return CommandResult::with_exit_code(
                            "".into(),
                            format!("split: invalid suffix length: '{}'\n", args[i + 1]),
                            1,
                        )
                    }
                }
                ...
                && arg[2..].chars().all(|c| c.is_ascii_digit())
            {
                ...
                additional_suffix = args[i + 1].clone();
                i += 2;
            } else if arg == "--" {
                ...
                return CommandResult::with_exit_code(
                    "".into(),
                    format!("split: unrecognized option '{}'\n", arg),
                    1,
                );
                ...
                if !"lbnad".contains(ch) {
                    return CommandResult::with_exit_code(
                        "".into(),
                        invalid_option("split", ch.encode_utf8(&mut [0u8; 4])),
                        1,
                    );
                }
                i += 1;
        ...
        let content = if input_file == "-" {
            ...
            let path = resolve_under_cwd(&ctx.cwd, input_file);
        ...
        for (idx, chunk) in file_chunks.iter().enumerate() {
            ...
            let path = resolve_under_cwd(&ctx.cwd, &filename);
...
fn split_by_lines(content: &str, lines_per_file: usize) -> Vec<String> {
    ...
    let lines: Vec<&str> = if has_trailing {
    ...
    for chunk_lines in lines.chunks(lines_per_file) {
        ...
        let chunk = if is_last && !has_trailing {
            joined
        } else {
            format!("{}\n", joined)
        };
...
mod tests {
    ...
    async fn test_split_default_prefix() {
        ...
        fs.write_file("/test.txt", "1\n2\n3\n".as_bytes())
            ...
            .unwrap();
        let r = SplitCommand
            .execute(make_ctx(vec!["-l", "1", "/test.txt"], "", fs.clone()))
            .await;
    ...
    async fn test_split_by_bytes() {
        let fs = Arc::new(InMemoryFs::new());
        fs.write_file("/test.txt", "abcdefghij".as_bytes())
            .await
            .unwrap();
        let r = SplitCommand
            .execute(make_ctx(vec!["-b", "4", "/test.txt"], "", fs.clone()))
            .await;
        assert_eq!(r.exit_code, 0);
    ...
    async fn test_split_by_bytes_attached() {
        ...
        fs.write_file("/test.txt", "abcdefghij".as_bytes())
            ...
            .unwrap();
        let r = SplitCommand
            .execute(make_ctx(vec!["-b5", "/test.txt"], "", fs.clone()))
            .await;
        assert_eq!(r.exit_code, 0);
    ...
    async fn test_split_chunks() {
        ...
        fs.write_file("/test.txt", "abcdefghij".as_bytes())
            ...
            .unwrap();
        let r = SplitCommand
            .execute(make_ctx(vec!["-n", "2", "/test.txt"], "", fs.clone()))
            .await;
        assert_eq!(r.exit_code, 0);
    ...
    async fn test_split_numeric() {
        ...
        fs.write_file("/test.txt", "1\n2\n3\n".as_bytes())
            ...
            .unwrap();
        let r = SplitCommand
            .execute(make_ctx(vec!["-d", "-l", "1", "/test.txt"], "", fs.clone()))
            .await;
        assert_eq!(r.exit_code, 0);
    ...
    async fn test_split_additional_suffix() {
        ...
        let r = SplitCommand
            .execute(make_ctx(
                vec!["--additional-suffix=.txt", "-l", "1", "/test.txt"],
                "",
                fs.clone(),
            ))
            .await;
    ...
    async fn test_split_missing_file() {
        let fs = Arc::new(InMemoryFs::new());
        let r = SplitCommand
            .execute(make_ctx(vec!["/nonexistent"], "", fs))
            .await;
        assert_eq!(r.exit_code, 1);
