...
pub use crate::commands::gzip_codec::{compress_gzip_default, decompress_gzip, is_gzip};
...
mod tests {
    ...
    fn test_gzip_round_trip() {
        ...
        let compressed = compress_gzip_default(data).unwrap();
    ...
    fn test_create_gzipped_archive() {
        ...
        let archive = create_archive(&[entry]);
        let compressed = compress_gzip_default(&archive).unwrap();
        assert!(is_gzip(&compressed));
    ...
    fn test_gzip_empty_data() {
        let compressed = compress_gzip_default(b"").unwrap();
        assert!(is_gzip(&compressed));
        let decompressed = decompress_gzip(&compressed).unwrap();
