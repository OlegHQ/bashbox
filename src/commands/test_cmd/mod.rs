...
fn cmp_i64(left: &str, right: &str, f: fn(i64, i64) -> bool) -> bool {
...
async fn evaluate_expression(args: &[&str], ctx: &CommandContext) -> bool {
    ...
    if args.len() == 3 {
        ...
        match op {
            ...
            "-eq" => return cmp_i64(left, right, |a, b| a == b),
            "-ne" => return cmp_i64(left, right, |a, b| a != b),
            "-lt" => return cmp_i64(left, right, |a, b| a < b),
            "-le" => return cmp_i64(left, right, |a, b| a <= b),
            "-gt" => return cmp_i64(left, right, |a, b| a > b),
            "-ge" => return cmp_i64(left, right, |a, b| a >= b),
            _ => {}
        }
    }
...
impl Command for BracketCommand {
    ...
    async fn execute(&self, ctx: CommandContext) -> CommandResult {
        ...
        // 移除 ] 后调用 test 逻辑
        let args: Vec<&str> = ctx.args[..ctx.args.len() - 1]
            .iter()
            .map(|s| s.as_str())
            .collect();
...
mod tests {
    ...
    async fn test_file_exists() {
        let ctx =
            make_ctx_with_files(vec!["-e", "/test.txt"], vec![("/test.txt", "content")]).await;
        let cmd = TestCommand;
        let result = cmd.execute(ctx).await;
        assert_eq!(result.exit_code, 0);
    }
    ...
    async fn test_is_file() {
        let ctx =
            make_ctx_with_files(vec!["-f", "/test.txt"], vec![("/test.txt", "content")]).await;
        let cmd = TestCommand;
        let result = cmd.execute(ctx).await;
        assert_eq!(result.exit_code, 0);
    }
    ...
    async fn test_is_not_directory() {
        let ctx =
            make_ctx_with_files(vec!["-d", "/file.txt"], vec![("/file.txt", "content")]).await;
        let cmd = TestCommand;
        let result = cmd.execute(ctx).await;
        assert_eq!(result.exit_code, 1);
    }
    ...
    async fn test_file_size_nonempty() {
        let ctx =
            make_ctx_with_files(vec!["-s", "/file.txt"], vec![("/file.txt", "content")]).await;
        let cmd = TestCommand;
        let result = cmd.execute(ctx).await;
        assert_eq!(result.exit_code, 0);
    }
