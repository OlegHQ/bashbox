...
fn make_fetch_fn(status: u16, body: &str, resp_headers: HashMap<String, String>) -> FetchFn {
...
async fn test_data_raw() {
    ...
    let ctx = make_ctx(
        vec!["--data-raw", "{\"key\":\"value\"}", "https://example.com"],
        Some(fetch),
    );
    let result = CurlCommand.execute(ctx).await;
    assert_eq!(result.exit_code, 0);
...
async fn test_verbose_post_request() {
    ...
    let ctx = make_ctx(
        vec!["-v", "-d", "data=test", "https://example.com"],
        Some(fetch),
    );
    let result = CurlCommand.execute(ctx).await;
    assert_eq!(result.exit_code, 0);
...
async fn test_upload_file_with_t_option() {
    let fs = Arc::new(InMemoryFs::new());
    fs.write_file("/upload.txt", b"file to upload")
        .await
        .unwrap();
...
async fn test_form_field_simple() {
    ...
    let ctx = make_ctx(
        vec!["-F", "field1=value1", "https://example.com"],
        Some(fetch),
    );
    let result = CurlCommand.execute(ctx).await;
    assert_eq!(result.exit_code, 0);
...
async fn test_network_error_exit_code() {
    let fetch: FetchFn = Arc::new(|_url, _method, _headers, _body| {
        Box::pin(async move { Err("Network access denied".to_string()) })
            as Pin<Box<dyn Future<Output = Result<FetchResponse, String>> + Send>>
    });
    let ctx = make_ctx(vec!["https://example.com"], Some(fetch));
    let result = CurlCommand.execute(ctx).await;
...
async fn test_method_not_allowed_exit_code() {
    let fetch: FetchFn = Arc::new(|_url, _method, _headers, _body| {
        Box::pin(async move { Err("HTTP method POST not allowed".to_string()) })
            as Pin<Box<dyn Future<Output = Result<FetchResponse, String>> + Send>>
    });
...
async fn test_redirect_error_exit_code() {
    let fetch: FetchFn = Arc::new(|_url, _method, _headers, _body| {
        Box::pin(async move { Err("Redirect target not in allow-list".to_string()) })
            as Pin<Box<dyn Future<Output = Result<FetchResponse, String>> + Send>>
    });
    let ctx = make_ctx(vec!["https://example.com"], Some(fetch));
    let result = CurlCommand.execute(ctx).await;
...
async fn test_too_many_redirects_exit_code() {
    let fetch: FetchFn = Arc::new(|_url, _method, _headers, _body| {
        Box::pin(async move { Err("Too many redirects".to_string()) })
            as Pin<Box<dyn Future<Output = Result<FetchResponse, String>> + Send>>
    });
    let ctx = make_ctx(vec!["https://example.com"], Some(fetch));
    let result = CurlCommand.execute(ctx).await;
...
async fn test_timeout_exit_code() {
    let fetch: FetchFn = Arc::new(|_url, _method, _headers, _body| {
        Box::pin(async move { Err("Request aborted".to_string()) })
            as Pin<Box<dyn Future<Output = Result<FetchResponse, String>> + Send>>
    });
    let ctx = make_ctx(vec!["https://example.com"], Some(fetch));
    let result = CurlCommand.execute(ctx).await;
...
async fn test_multiple_data_options() {
    ...
    let ctx = make_ctx(
        vec!["-d", "first", "-d", "second", "https://example.com"],
        Some(fetch),
    );
    let result = CurlCommand.execute(ctx).await;
    assert_eq!(result.exit_code, 0);
...
async fn test_auth_header_encoding() {
    ...
    let ctx = make_ctx(
        vec!["-u", "admin:secret123", "https://example.com"],
        Some(fetch),
    );
    let result = CurlCommand.execute(ctx).await;
    assert_eq!(result.exit_code, 0);
...
async fn test_form_with_special_characters() {
    ...
    let ctx = make_ctx(
        vec!["-F", "field=value with spaces", "https://example.com"],
        Some(fetch),
    );
    let result = CurlCommand.execute(ctx).await;
    assert_eq!(result.exit_code, 0);
...
async fn test_cookie_jar_with_multiple_cookies() {
    ...
    headers.insert(
        "set-cookie".to_string(),
        "cookie1=value1; cookie2=value2".to_string(),
    );
    let fetch = make_fetch_fn(200, "ok", headers);
    let fs = Arc::new(InMemoryFs::new());
    let ctx = make_ctx_with_fs(
...
async fn test_explicit_get_with_data() {
    ...
    let ctx = make_ctx(
        vec!["-X", "GET", "-d", "data", "https://example.com"],
        Some(fetch),
    );
    let result = CurlCommand.execute(ctx).await;
    assert_eq!(result.exit_code, 0);
...
async fn test_form_switches_to_post() {
    ...
    let ctx = make_ctx(
        vec!["-F", "field=value", "https://example.com"],
        Some(fetch),
    );
    let result = CurlCommand.execute(ctx).await;
    assert_eq!(result.exit_code, 0);
...
async fn test_generic_error_exit_code() {
    let fetch: FetchFn = Arc::new(|_url, _method, _headers, _body| {
        Box::pin(async move { Err("Generic error".to_string()) })
            as Pin<Box<dyn Future<Output = Result<FetchResponse, String>> + Send>>
    });
    let ctx = make_ctx(vec!["https://example.com"], Some(fetch));
    let result = CurlCommand.execute(ctx).await;
